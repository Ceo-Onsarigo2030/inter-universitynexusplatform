-- Reconnect public frontend policies to published/approved data without requiring admin helper evaluation for visitors.
...
-- Keep voter fingerprints private while allowing the public leaderboard to work.
...
-- Public aggregate counters without exposing profile rows.
...
-- Helper for one-time backfill scripts if any existing auth users are missing profiles.
